...
public class TupleDesc implements Serializable {
    ...
    public int fieldNameToIndex(String name) throws NoSuchElementException {
        for(int i = 0; i < tupledesc.length; i ++){
            if(tupledesc[i].fieldName != null && tupledesc[i].fieldName.equals(name)){
                return i;
            }
        }
        throw new NoSuchElementException();
    ...
    public int getSize() {
        int result = 0;
        for(int i = 0; i < tupledesc.length; i++){
            result += tupledesc[i].fieldType.getLen();
        }
        return result;
    }
    ...
    public Iterator<TDItem> iterator() {
        ...
        List<TDItem> tdList = Arrays.asList(tupledesc);
    ...
    public boolean equals(Object o) {
        ...
        if(other.tupledesc.length != this.tupledesc.length){
            return false;
        }
        for(int i = 0; i < tupledesc.length;i++){
            if(tupledesc[i].fieldType != other.tupledesc[i].fieldType){
                return false;
            }
        }
        return true;
    ...
    public String toString() {
        String result = "";
        for(int i = 0; i < tupledesc.length - 1;i++){
            result += tupledesc[i].toString();
            result += ", ";
        }
        result += tupledesc[tupledesc.length - 1].toString();
        return result;
    }
    ...
    public static TupleDesc merge(TupleDesc td1, TupleDesc td2) {
        TDItem[] result = new TDItem[td1.tupledesc.length + td2.tupledesc.length];
        for(int i = 0; i < td1.tupledesc.length; i ++){
            result[i] = td1.tupledesc[i];
        }
        for(int i = td1.tupledesc.length; i < result.length; i++){
            result[i] = td2.tupledesc[i-td1.tupledesc.length];
        }
